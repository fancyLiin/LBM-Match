...
class SimilarityDataset(Dataset):
    def __init__(self, data_file, max_seq_length=float('inf')):
        self.data = []
        self.max_seq_length = max_seq_length if isinstance(max_seq_length, int) else 0
...
# 主函数
def main():
    dataset = SimilarityDataset('datacossim.json')  # 假设最大序列长度为10
    tensorlist = []
    labellist = []
    # 检查前几个样本的处理结果
    for i in range(min(32, len(dataset))):  # 查看前5个样本
        seq, label = dataset[i]
        # print(f"Sample {i+1}:")
